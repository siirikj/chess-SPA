...
const moveChessPiece = (
  ...
  const uppdatedPiecesLocation = { ...piecesLocation };
  uppdatedPiecesLocation[activePlayer][chessPiece].position.number =
    9 - parseInt(boardNumber, 10);
  uppdatedPiecesLocation[activePlayer][chessPiece].position.letter =
    boardLetter;
...
const checkForChessPieceBetweenHorizontalVertical = (
  ...
  // 1 if up, -1 if down
  const upOrDown = distanceNumbers > 0 ? 1 : -1;
  const absOrNotVertical =
    distanceNumbers > 0 ? distanceNumbers : Math.abs(distanceNumbers);
  // -1 if left, 1 if right
  const rightOrLeft = distanceLetters > 0 ? -1 : 1;
  const absOrNotHorizontal =
  ...
  blackPieces.forEach((chessPiece) => {
    for (let i = 1; i < stopValueLoop; i += 1) {
...
const legalMove = (
  ...
  // Variabled
  const currentNumber =
    piecesLocation[activePlayer][chessPiece].position.number;
  const currentLetter =
    piecesLocation[activePlayer][chessPiece].position.letter;
  ...
  if (chessPiece === "king") {
    ...
    if (Math.abs(distanceLetters) !== 0 || Math.abs(distanceNumbers) > 1) {
...
const Square = ({
  ...
  if (
    selectedSquare.number === parseInt(boardNumber, 10) &&
    ...
    <button
      ...
      onClick={() => {
        ...
        if (
          ...
          const newSelectedSquare = {
            number: parseInt(boardNumber, 10),
            letter: boardLetter,
          ...
          if (
            ...
            if (piceOnTop === "") {
              ...
              const opponentColor =
                activePlayer === "black" ? "white" : "black";
      ...
      {boardNumbers.map((boardNumber, rowIdx) => (
        <div className="flex" key={boardNumber}>
          {boardLetters.map((boardLetter, colIdx) => {
            ...
            if (pawnInfo[0] !== "") {
              const [piceOnTopTemp, unicodePicTemp] = pawnInfo;
              piceOnTop = `B${piceOnTopTemp}`;
              unicodePic = unicodePicTemp;
            }
